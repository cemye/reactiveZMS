...
def showPlot():
    ...
    if save_plots:
        plt.savefig("plots\predictive_plot" + str(plot_number) + ".png")
        plot_number += 1
...
def optimize(schedule, reactive):
    ...
    fit = fitness(encode(decoded), schedule)  # schedule anstelle decoded
    currentBestScore = fit
    return decoded
...
def calc_scores(schedule):  # todo
    ...
    if methode == "lateness":
        ...
        if (schedule['timestamp'] - schedule['desired_timestamp']).abs() > 0:
            schedule['score'] = 1
        else:
            schedule['score'] = 0
    else:
        print("No proper method has been given!")
...
DATASET = "lager_gross"  # @param ["lager_gross", "lager_klein", "lager_mittel"]
PATH = r"C:\Users\cemye\OneDrive - uni-oldenburg.de\Bachelorarbeit\reactive ZMS"

# Wähle Bewertungsfunktion für Zeitfenster: lateness, tardy
methode = "lateness"
...
save_plots = True
# Gewicht für den Erhaltungsfakor der Bewertungsfunktion
weight = 0
runs = 20
...
"""--------------------------------prädiktive Ablaufplanung--------------------------------"""

"""Erstellt einen Ablaufplan prädiktiv"""
def predictiveScheduling():
    global fig, gnt, schedule
...
for i in range(runs):
    print(i)
    fit = predictiveScheduling()
    solutions.append(np.round(fit, 2))
